...
import { X, Plus, AlertCircle, Edit2, ChevronDown } from 'lucide-react';

/**
 * EducationSection Component
 * - Display extracted education with confidence indicators
 * - Allow adding, editing, and deleting education entries
...
const EducationSection = ({ education = [], onEdit, onDelete }) => {
  ...
  const handleEditEducation = (index) => {
    onEdit?.({
      ...
      data: {
        ...editValues,
        confidence: education[index]?.confidence,
      },
    });
    setEditingId(null);
    setEditValues({});
    ...
    <div className="border rounded-lg p-6 bg-gradient-to-br from-purple-50 to-white">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-1 text-purple-600 hover:bg-purple-50 rounded transition-colors"
          disabled={isAdding}
        >
          ...
          education.map((edu, index) => (
            <div
              ...
              className={`border rounded-lg p-4 ${
                edu.confidence && edu.confidence < 0.8
            ...
            >
              {editingId === index ? (
                <div className="space-y-3">
                  <input
                    type="text"
                    value={editValues.degree || edu.degree}
                    onChange={(e) =>
                      setEditValues({ ...editValues, degree: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
                  />
                  <input
                    type="text"
                    value={editValues.institution || edu.institution}
                    onChange={(e) =>
                      setEditValues({
                        ...editValues,
                  ...
                  <input
                    type="number"
                    value={editValues.year || edu.year}
                    onChange={(e) =>
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      onClick={() => onDelete?.({ type: 'education', index })}
      ...
      <p className="text-sm text-gray-500 mt-4">
        💡 You can add, edit, or remove education entries. Items with low confidence are highlighted.
      </p>
    </div>
  );
};
